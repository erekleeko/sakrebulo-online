...
function initAdmin() {
    // შენი გაშვებული Apps Script-ის ბმული
    const GAS_URL = "https://script.google.com/macros/s/AKfycbziGCr2dWBp5l8oB8uMGalv3aJdII7BwPcZjT-0RNO8an4iFzT7vOiSw7mA15ziaMd2uA/exec";

    window.syncFromDrive = async function() {
        const btn = document.getElementById("syncBtn");
        btn.innerText = "მიმდინარეობს სინქრონიზაცია..."; 
        btn.disabled = true;

        try {
            // 1. მოგვაქვს ფაილები Drive-დან
            const response = await fetch(GAS_URL);
            const files = await response.json();

            if(files.length === 0) {
                alert("Google Drive-ის საქაღალდე ცარიელია!");
                btn.innerText = "🔄 დრაივიდან სინქრონიზაცია"; 
                btn.disabled = false;
                return;
            }

            // 2. ვკითხულობთ ბაზაში უკვე არსებული ფაილების ბმულებს (რომ დუბლიკატი არ შეიქმნას)
            const snapshot = await getDocs(issuesRef);
            const existingUrls = new Set();
            snapshot.forEach(doc => existingUrls.add(doc.data().fileUrl));

            let addedCount = 0;

            // 3. ვამატებთ ახალ საკითხებს ბაზაში
            for (const file of files) {
                let fileUrl = file.url;
                
                // ვამზადებთ ლინკს ისე, რომ iframe-ში პირდაპირ წასაკითხად გაიხსნას
                ...
                // თუ ეს დოკუმენტი ბაზაში ჯერ არ არის, ვამატებთ
            ...
            alert(`სინქრონიზაცია დასრულდა! დაემატა ${addedCount} ახალი საკითხი.`);
        } catch (error) {
            console.error("შეცდომა სინქრონიზაციისას:", error);
